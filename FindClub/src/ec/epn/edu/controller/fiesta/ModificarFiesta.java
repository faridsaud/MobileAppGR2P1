	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (redireccion == true) {
			...
			if (combos == null || combos.equals("si")) {
				...
				int idFiesta = 0;
				try {
					idFiesta = Integer.parseInt(request.getParameter("fModificar"));
					fiesta = sf.buscarFiesta(fiesta, idFiesta);
					request.getSession().setAttribute("fiestaModificar", fiesta);

				} catch (Exception e) {
					System.out.println("Error al modificar fiesta, se recibió:" + request.getParameter("fModificar"));
				}
				...
				getServletConfig().getServletContext().getRequestDispatcher("/vistas/fiesta/modificar.jsp")
